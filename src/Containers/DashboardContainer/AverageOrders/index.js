...
const AverageOrders = (props) => {
	...
	const getData = () => {
		...
		temp = props.listOfOrders.ordersGraphData.map((item, index) =>
			item.period.toString().length > 3
				? item.period.toString().substr(0, 3)
	...
	const getDifference = () => {
		if (!listOfOrders) {
			...
			console.log(
				"avgOrders[0].avgOrders && previousAvgOrders[0].previousAvgOrders" +
					avgOrders[0].avgOrders +
					" --" +
					previousAvgOrders[0].previousAvgOrders
			);
			...
			if (avgOrders[0].avgOrders && !previousAvgOrders[0].previousAvgOrders) {
				...
				let reslt =
					parseFloat(
						listOfOrders.previousAvgOrders[0].previousAvgOrders
					).toFixed(2) -
					parseFloat(listOfOrders.avgOrders[0].avgOrders).toFixed(2);
				reslt =
					reslt /
					parseFloat(
						listOfOrders.previousAvgOrders[0].previousAvgOrders
					...
					return (
						<>
							...
							<Typography
								...
								fontFamily="Gilroy-Medium"
								color="#10B981"
								title={parseFloat(reslt).toFixed(2) + "%"}
							/>
						</>
					);
				} else {
					return (
						<>
							...
							<Typography
								...
								fontFamily="Gilroy-Medium"
								color="red"
								title={parseFloat(reslt).toFixed(2) * -1 + "%"}
							/>
						</>
					);
				}

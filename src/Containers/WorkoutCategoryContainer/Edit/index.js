...
import { updateWorkoutCategory } from "../../../Helpers/firebase";
const { TextArea } = Input;

function Edit({ setEditModal, activeCategory }) {
	const [addingUser, setAddingUser] = React.useState(false);
	const [imageurl, setImageUrl] = React.useState(activeCategory.image ?? "");
	...
	const AddUserToList = async (values) => {
		...
		values.id = activeCategory.id;
		if (await updateWorkoutCategory(values)) {
			notification.success({
				message: `Successfully updated Exercise category!`,
				description: `${values.name}  has been successfully updated and saved to Exercise Category list`,
				placement: "topRight",
				duration: 3,
				onClose: function () {
					setAddingUser(false);
